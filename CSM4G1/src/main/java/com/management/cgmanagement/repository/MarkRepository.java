...
public interface MarkRepository extends JpaRepository<Mark,Long> {
    ...
    @Query(nativeQuery = true,value = "select `users`.`full_name`,`users`.`date_of_birth`,`users`.`address`,`users`.`email`,`users`.`phone_number`,`classes`.`name_class` from users join user_roles on `users`.`id` = `user_roles`.`role_id` join user_class on `users`.`id` = `user_class`.`user_id` join classes on `user_class`.`class_room_id` = `classes`.`id`  ;")
